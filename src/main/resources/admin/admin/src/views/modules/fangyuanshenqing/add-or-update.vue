  ...
  methods: {
    ...
    init(id,type) {
      ...
      this.$http({
		...
		if(json.hunyin!=''&&json.hunyin){
              		this.ruleForm.hunyin = json.hunyin
		}
        } else {
	...
	this.ruleForm.shenqingneirong = this.ruleForm.shenqingneirong.replace(reg,'../../../springbooth6iu7/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `fangyuanshenqing/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.fangyuanshenqingCrossAddOrUpdateFlag = false;
                  this.parent.search();
